...
@keyframes floatBouquet {
  ...
  50% { opacity: 1; }
  100% { transform: translateY(-20%) rotate(20deg); opacity: 1; }
}
.bouquet {
  position: fixed;
  bottom: 0;
  width: 50px;
  pointer-events: none;
  animation: floatBouquet 8s linear infinite;
}
...
function moveNo(px, py) {
  ...
  dx /= mag; dy /= mag;
  let moveAmount = 120 + Math.random()*80;
  let newLeft = (b.left - z.left) + dx * moveAmount;
  let newTop  = (b.top - z.top) + dy * moveAmount;
...
zone.addEventListener("pointermove", e => checkProximity(e.clientX, e.clientY));
zone.addEventListener("touchmove", e => { const touch = e.touches[0]; checkProximity(touch.clientX, touch.clientY); });
...
function showHeart() { popHeart.classList.add('pop'); setTimeout(() => popHeart.classList.remove('pop'), 1000); }
...
/* ---------- FLOATING HEARTS ---------- */
const heartColors = ["#ff4d7d","#ff1f68","#ff80ab","#ff99cc"];
const maxHearts = 30;
let hearts = [];
function createHeart() {
  ...
  hearts.push(heart);
  setTimeout(() => { heart.remove(); hearts = hearts.filter(h => h !== heart); }, 5000);
}
setInterval(createHeart, 400);

/* ---------- FLOATING BOUQUETS ---------- */
const bouquetImages = [
  "https://i.ibb.co/Jv5y1Cg/bouquet1.png", 
  "https://i.ibb.co/7rP1Lf1/bouquet2.png"
]; // replace with any small bouquet png links
function createBouquet() {
  const img = document.createElement('img');
  img.src = bouquetImages[Math.floor(Math.random()*bouquetImages.length)];
  img.classList.add('bouquet');
  img.style.left = Math.random() < 0.5 ? "-60px" : "calc(100% + 60px)";
  img.style.width = 40 + Math.random()*30 + "px";
  document.body.appendChild(img);
  img.animate([
    { transform: `translateY(120%) rotate(0deg)`, opacity: 0 },
    { transform: `translateY(-20%) rotate(${Math.random()*30-15}deg)`, opacity: 1 }
  ], { duration: 8000, easing: 'linear' });
  setTimeout(() => img.remove(), 8000);
}
setInterval(createBouquet, 3000);
